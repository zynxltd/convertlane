...
<x-layouts.app :title="$title" description="Publisher onboarding questionnaire." robots="noindex, nofollow">
    <x-page-hero eyebrow="Onboarding">
        <x-slot:title>Publisher <span class="text-gradient-hero">questionnaire</span></x-slot:title>
        <x-slot:subtitle>
            Complete this to start due diligence. You must be able to provide government-issued photo ID (passport/driving licence/national ID) and proof of address.
        </x-slot:subtitle>
    </x-page-hero>
            ...
            <div class="glass rounded-2xl p-8">
                ...
                @if ($questionnaireComplete ?? false)
                    <div class="space-y-4 text-sm text-muted">
                        ...
                        <a href="{{ route('onboarding.publisher.agreement', ['email' => request('email', $prefill['contact_email'] ?? ''), 'ref' => request('ref', $prefill['partner_reference'] ?? '')]) }}" class="btn-primary inline-flex">Continue to agreement</a>
                    </div>
                @else
                    <form
                        action="{{ route('onboarding.publisher.store') }}"
                        method="POST"
                        class="relative"
                        novalidate
                    ...
                    >
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="grid gap-5 sm:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <select class="form-input @error('entity_type') border-red-500 @enderror" id="entity_type" name="entity_type" required>
                                        ...
                                        <option value="individual" @selected($et === 'individual')>Individual (not registered)</option>
                                    </select>
                            ...
                            <label class="mt-2 flex items-start gap-3 text-sm text-muted">
                                ...
                                <span>I confirm I can provide government-issued photo ID (passport / driving licence / national ID) and proof of address to proceed with due diligence.</span>
                            </label>
